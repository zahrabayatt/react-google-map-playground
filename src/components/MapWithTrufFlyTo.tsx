    ...
    const animate = () => {
      if (currentStep < steps) {
        ...
        const point = turf.along(
          ...
          (currentStep / steps) * turf.length(turf.lineString(coordinates))
        );
  ...
  return (
    <div style={{ height: "100vh", width: "100%" }}>
      <div style={{ position: "absolute", top: 10, left: 10, zIndex: 1000 }}>
        <button
          onClick={() =>
            flyToLocation(locations.location1, locations.location2)
          }
        >
          Fly to Los Angeles
        </button>
        <button
          onClick={() =>
            flyToLocation(locations.location2, locations.location1)
          }
        >
